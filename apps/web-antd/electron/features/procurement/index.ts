...
export class ProcurementAnalyzer {
  static async run({
    ...
    const refData = await readExcel(refBuffer);
    ...
    const summary = `处理完成！共扫描 ${listData.length} 条数据，保留 ${keptCount} 条，已移除 ${
