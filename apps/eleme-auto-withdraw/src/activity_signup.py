...
class Config:
    """配置类"""
    # 饿了么活动页面URL
    ACTIVITY_URL = "https://nr.ele.me/app/eleme-nr-bfe-newretail/common-next#/pc/activityCenter"
    
    # 截图保存路径
    SCREENSHOT_DIR = Path("screenshots")
    
    # 日志文件
    LOG_FILE = Path("activity_signup.log")
    ...
    # 活动关键词
    ACTIVITY_KEYWORDS = ["报名", "立即报名", "参与活动", "领取"]
    
    # 门店名称
    STORE_NAMES = ["Oby便利超市(安吉店)", "Oby便利超市(长兴店)"]
...
class ActivitySignupAutomation:
    ...
    def find_activity_buttons(self, image_path: Path) -> List[Tuple[int, int, int, int]]:
        ...
        # 使用OCR或模板匹配查找按钮
        # 这里使用简单的文字检测（实际项目中可以使用Pytesseract或更高级的OCR）
        
        buttons = []
        
        # 方法1：基于颜色的按钮检测（假设报名按钮是特定颜色）
        # 转换到HSV颜色空间
        ...
        # 方法2：基于文字模板匹配（如果有按钮模板）
        # template_path = Path("templates/signup_button.png")
        ...
        #         buttons.append((pt[0], pt[1], template.shape[1], template.shape[0]))
