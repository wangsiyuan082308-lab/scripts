...
- 商品比对不再把 `UPC` 不一致直接视为高风险未匹配。
- `productMaster` 模式调整为：
  - `UPC` 精确匹配
  - 历史 AI 命中复用
  - 规则匹配
  - AI 兜底
- 修复了“先本地候选筛空，AI 根本没有参与”的问题。

#### 规则匹配增强

- 增强了名称/规格/量纲特征评分。
- 支持处理“同品但包装口径不同”的情况，例如单瓶和整箱的差异。
- 修复了商品总表按门店展开后，同一商品多条候选互相挤掉规则命中的问题。

#### AI 命中沉淀为项目记忆

- 新增商品比对历史命中记忆。
- 当 AI 首次确认某个非 `UPC` 同品关系后，后续相同名称/规格模式会直接命中，不再重复请求 AI。
- 该记忆保存在后端运行时目录，不写入仓库。

#### 运行时配置与部署

- 商品比对 AI 配置改为通过后端运行时配置读取。
- 百炼配置已通过服务端配置接口写入线上运行环境。
- 后端服务已热更新，线上已验证 `农夫山泉尖叫蓝色多肽型运动饮料550ml` 可以命中商品总表中的对应商品。

#### 相关文档

- `docs/PROJECT_MEMORY.md`
- `docs/PRODUCT_COMPARE_MEMORY.md`
