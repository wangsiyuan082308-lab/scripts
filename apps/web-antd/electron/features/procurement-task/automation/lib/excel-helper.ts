 ...
 * 自动识别前 10 行中的表头
 */
export async function readExcel(buffer: any): Promise<any[]> {
 ...
 * 从文件路径读取 Excel
 */
export async function readExcelFromFile(filePath: string): Promise<any[]> {
  const buffer = fs.readFileSync(filePath);
  return readExcel(buffer as Buffer);
}
